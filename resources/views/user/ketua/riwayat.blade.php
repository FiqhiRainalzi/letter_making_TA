                ...
                <div class="card">
                    <div class="card-body">
                        <form id="deleteAllForm" action="{{ route('riwayat.destroyAll') }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="button" onclick="confirmDeleteAll()" class="btn btn-danger mb-3 mt-1">
                                Hapus Semua Riwayat
                            </button>
                        </form>
                                ...
                                @forelse($riwayats as $index => $item)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            {{ $item->ajuanSurat->getSurat()?->nomorSurat ?? 'belum diberi nomor' }}
                                        </td>
                                        <td>{{ $item->catatan }}</td>
                                        <td>
                                            <form id="deleteForm-{{ $item->id }}" action="{{ route('riwayat.destroy', $item->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
